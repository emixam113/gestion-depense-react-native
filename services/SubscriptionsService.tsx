...
export const SubscriptionsProvider = ({ children, userId }: any) => {
  ...
  useEffect(() => {
    const initStatus = async () => {
      try {
        ...
        // 2. Configurer RevenueCat avec ton ID 6
        if (userId) {
        ...
        console.error("Erreur Provider:", e);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <SubscriptionContext.Provider value={{ isPremium, loading, setIsPremium: (v) => setIsPremium(v) }}>
      {children}
